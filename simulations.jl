...
@everywhere function simulate(N::Int64,
    ...
    C_pop = Dict( "partial" => zeros(Int64, nr_ind,T))
    W_pop = Dict( "partial" => zeros(nr_ind,T))
    R_pop = Dict( "partial" => zeros(nr_ind,T))
...
rho_vals = [0.0, 0.5, 0.9]#[0.0, 0.1, 0.3, 0.5, 0.7, 0.9]
# utility idiosyncratic degree 
beta_vals = [0.0, 0.8, 5.]#[0.0, 0.4, 0.8, 1., 2., 5.]
# absolute risk aversion
alpha_vals = [0.0, 0.6, 5.]#[0.0, 0.3, 0.6, 1., 5.]

sigma_vals = [0.25, 1.0, 4.0]#[0.25, 0.5, 1.0, 2.0, 4.0]
...
global total_num = 0
global para_counter = 0
for (N, T, rho, beta, sigma, alpha) in params
    println(para_counter)
    global para_counter = para_counter + 1
    println("STARTING")
    ...
    if total_num > NUM_SIMS_TO_WRITE
        file_name = string("julia_sim_",file_idx,".json")
